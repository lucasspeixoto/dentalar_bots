...
from core.whatsapp_scrapping import WhatsAppScrapping
...
from help import * # CONTAIN STRING VARIABLE CONTAINING THE ABOUT OF EACH PAGE IN THE APPLICATION
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.setWindowTitle(application_name) #SETS THE APPLICATION NAME IN THE WINDOW TOPBAR                        ---------(C4)
        ...
        UIFunction.labelTitle(self, application_name) #PASSING THE CODE TO SET THE TITLE TO THE CUSTOME TOPBAR IN OUR UI
        ...
        self.ui.bn_whatsapp.clicked.connect(lambda: UIFunction.buttonPressed(self, 'bn_whatsapp'))
        ...
        self.contacts.selected_image = ''
            ...
            #--Load Contacts----------------------:
        self.ui.load_contacts_button.clicked.connect(lambda: threading.Thread(target=Contacts.load_contacts(self), daemon=True).start())
        
            #-- Check Load All Contacts Button
        self.ui.send_to_all_contacts_checkbox.toggled.connect(lambda: threading.Thread(target=Contacts.toggle_all_contacts(self), daemon=True).start())

            #-- Clear All Contacts in the list Button
        self.ui.clear_contacts_list_button.clicked.connect(lambda: threading.Thread(target=Contacts.clear_contacts_list(self), daemon=True).start())
        
            #-- Send messages
        self.ui.send_whatsapp_messages_button.clicked.connect(lambda: threading.Thread(target=Contacts.send_whatsapp_messages(self), daemon=True).start())
        
            #-- Load Image
        self.ui.load_image_button.clicked.connect(lambda: threading.Thread(target=Files.load_image(self), daemon=True).start())
